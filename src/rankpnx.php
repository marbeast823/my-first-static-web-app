...
$result_online = mssql_query("SELECT SUM(P.ChaOnline) AS kiraonline FROM RanGameS1.dbo.ChaInfo P");
while ($row = mssql_fetch_array($result_online)) {
    $userOnline = $row["kiraonline"];
}
$result_cha = mssql_query("SELECT COUNT(*) AS kirachar FROM RanGameS1.dbo.ChaInfo P");
while ($row1 = mssql_fetch_array($result_cha)) {
    $chaCount = $row1["kirachar"];
}
$result_user = mssql_query("SELECT COUNT(*) AS kirauser FROM RanUser.dbo.GSUserInfo U");
while ($row2 = mssql_fetch_array($result_user)) {
    $userCount = $row2["kirauser"];
}

$result_Block = mssql_query("SELECT COUNT(*) AS kirauser FROM RanUser.dbo.GSUserInfo U where u.userblock=1");
...
<head>
	<title><?php require("config.php"); echo($web['webtitle']); ?> - <?php if(!isset($_GET['op'])){echo("");} else{echo(ucfirst($_GET['op']));} ?> Hybrid Phoenix Rankings</title>
	...
	<div class="page">
		...
		<div class="contside">
			<ul class="lnb_menu">
				...
				<li><div class="icons ico_lnb"></div><a href="ranksg.php" title="Hot Items">Sacred Gate Rank</a></li>
				<li><div class="icons ico_lnb"></div><a href="rankmp.php" title="Hot Items">Mystic Peak Rank</a></li>
				<li><div class="icons ico_lnb"></div><a href="rankpnx.php" title="Hot Items" class="selected">Phoenix Rank</a></li>
...
<div style="padding:10px 0;"><center><a href="http://ranindonesia.com/c3-premium-items"><img src="images/premium_title.png" /></a></center></div>
                  ...
                  <table><? include_once("modules/pnx.php"); ?></table>
</div>
