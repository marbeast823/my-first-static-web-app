     ...
     if ((isset($_SESSION['pass'])) && (isset($_SESSION['user'])));
           {
               ...
               require("config.php");
               require("includes/validate.class.php");
	       $_POST['oldpassword'] = strtolower($_POST['oldpassword']);
               $login = stripslashes($_SESSION['user']);
               ...
               $online_check = $db->Execute("SELECT UserAvailable FROM gsuserinfo WHERE username=?",array($login));
               $row2 = $online_check->fetchrow();

                  if($web['md5']==0){
                        $sql_pw_check = $db->Execute("SELECT * FROM gsuserinfo WHERE  Username=? AND   Userpass = ?",array($login,$oldpwd,$UserPass));
                                      }
                  elseif($web['md5']==0){
                        $sql_pw_check = $db->Execute("SELECT * FROM gsuserinfo WHERE  Username=? AND   Userpass=?",array($login,$oldpwd,$UserPass));
                                          }
	          ...
	          $elems[] = array('name'=>'oldpassword', 'label'=>''.$warning_start.' Current Password Is Invalid (4-10 Alpha-Numeric Characters) '.$warning_end.'', 'type'=>'text', 'required'=>true, 'len_min'=>4,'len_max'=>32, 'cont' =>'alpha');
	          ...
	          $elems[] = array('name'=>'renewpassword', 'label'=>''.$warning_start.' Passwords Did not Match '.$warning_end.'','type'=>'text', 'required'=>true, 'len_min'=>4,'len_max'=>14, 'cont' =>'alpha','equal'=> array('newpassword'));
				 ...
				 $newpwd = strtolower($newpwd);
               			 $login = stripslashes($_SESSION['user']);
                                 mssql_query ("UPDATE gsuserinfo SET userpass='$newpwd' WHERE username ='$login'");
     ...
     if ((isset($_SESSION['pass'])) && (isset($_SESSION['user'])));
           {
               ...
               $row2 = $online_check->fetchrow();
                  ...
                  if($web['md5']==0){
                        $sql_pw_check = $db->Execute("SELECT * FROM gsuserinfo WHERE  Username=? AND   Userpass = ?",array($login,$oldpwd));
                                      }
                  elseif($web['md5']==0){
                        $sql_pw_check = $db->Execute("SELECT * FROM gsuserinfo WHERE  Username=? AND   Userpass=?",array($login,$oldpwd));
                                          }
   		 if($web['md5']==0){
                        $sql_pw2_check = $db->Execute("SELECT * FROM gsuserinfo WHERE  Username=? AND   Userpass2=?",array($login,$userpass2));
                                      }
                  elseif($web['md5']==0){
                        $sql_pw2_check = $db->Execute("SELECT * FROM gsuserinfo WHERE  Username=? AND   Userpass2=?",array($login,$userpass2));
                                          }
	          ...
	          $elems[] = array('name'=>'oldpassword', 'label'=>''.$warning_start.' Current Password Is Invalid (4-10 Alpha-Numeric Characters) '.$warning_end.'', 'type'=>'text', 'required'=>true, 'len_min'=>4,'len_max'=>32, 'cont' =>'alpha');
	          $elems[] = array('name'=>'UserPass2', 'label'=>''.$warning_start. 'PinCode Is Invalid (4-10 Alpha-Numeric Characters) '.$warning_end.'', 'type'=>'text', 'required'=>true, 'len_min'=>4,'len_max'=>14, 'cont' =>'alpha');
	          $elems[] = array('name'=>'newpassword', 'label'=>''.$warning_start. 'New PinCode Is Invalid (4-10 Alpha-Numeric Characters) '.$warning_end.'', 'type'=>'text', 'required'=>true, 'len_min'=>4,'len_max'=>14, 'cont' =>'alpha');
	          $elems[] = array('name'=>'renewpassword', 'label'=>''.$warning_start.' NewPinCode Did not Match '.$warning_end.'','type'=>'text', 'required'=>true, 'len_min'=>4,'len_max'=>14, 'cont' =>'alpha','equal'=> array('newpassword'));
		   ...
		   if ($UserPass2 <= 0){$error = 1; 
                           show_error("$warning_start PinCode Is Incorrect! $warning_end"); 
                  }
                  ...
                  if($error!=1){	
               			 ...
               			 $login = stripslashes($_SESSION['user']);
               			 $UserPass2 = ($UserPass2);
				 mssql_query ("UPDATE RanUser.dbo.GSUserinfo SET UserPass2 = '$newpwd' WHERE Username = '$login'");
 
                                    
                                           $_SESSION['pass'] = $newpwd;
                                               show_error("$ok_start PinCode SuccessFully Changed! $ok_end");
                  }
