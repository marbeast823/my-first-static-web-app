...
<?php
	...
	*/
	...
	$total_pages = mssql_fetch_array(mssql_query($query));
	$total_pages = $total_pages[num];
	
	/* Setup vars for query. */
	$targetpage = "itemshop.php?op=funtional"; 	//your file name  (the name of this file)
	...
	if($page == $lastpage1)
			{
			If ($sobra3 == 0)
			$item_category = mssql_query("SELECT * FROM (SELECT TOP $limit  * FROM (SELECT TOP $start * FROM $tbl_name WHERE Category =  $type ORDER BY ProductNum DESC) AS select_limit_rev ORDER BY ProductNum ASC ) AS select_limit WHERE Category =  $type ORDER BY ProductNum DESC");		
			else
						$item_category = mssql_query("SELECT * FROM (SELECT TOP $sobra3  * FROM (SELECT TOP $start * FROM $tbl_name WHERE Category =  $type ORDER BY ProductNum DESC) AS select_limit_rev ORDER BY ProductNum ASC ) AS select_limit WHERE Category =  $type ORDER BY ProductNum DESC");		
			}
	else{
	$item_category = mssql_query("SELECT * FROM (SELECT TOP $limit  * FROM (SELECT TOP $start * FROM $tbl_name WHERE Category =  $type ORDER BY ProductNum DESC) AS select_limit_rev ORDER BY ProductNum ASC ) AS select_limit WHERE Category =  $type ORDER BY ProductNum DESC");		
	}
...
if($_POST['lostpassword']=='Buy') {
	...
	$result = mssql_query ("SELECT ItemMain,ItemSub,ItemName,ItemMoney,ItemStock FROM Ranshop.dbo.ShopItemMap Where ProductNum = '$ItemNum'");
	...
	$result1=mssql_query("SELECT Username, UserPoint, UserNum FROM Ranuser.dbo.GSUserinfo Where Username = '$account_id'");
...
$check_id_valid = mssql_query ("SELECT ItemMain,ItemSub,ItemName,ItemMoney,ItemStock FROM Ranshop.dbo.ShopItemMap Where ProductNum = '$ItemNum'");
...
jump('webshop.php'); Die ('<script language="JavaScript">alert();</script>'); 
	...
	mssql_query ("UPDATE Ranuser.dbo.GSUserinfo SET UserPoint = UserPoint - $ItemMoney WHERE Username = '$account_id'");
...
$perRow=2;
$r=0;
$resTop1 = mssql_query("SELECT ProductNum, ItemMain, ItemSub, Duration, ItemStock, ItemMoney, ItemImage,ItemName,ItemComment FROM RanShop.dbo.ShopItemMap where Category = '6'");
while($result1 = mssql_fetch_array( $item_category)) {
		$r++;	
            ...
            <table width='100%' border='0' >
              <tr>
                <td valign='top' width='90'><table>
                                        <tr>
                      <td style='border-bottom:none;'><img id='ctl00_ContentPlaceHolder_main_MartList_ctl00_wupinpic' class='bbb' src='items/$ItemImage1' rel='facebox' style='height:100px;width:100px;border-width:0px;' />
