...
if($_POST['lostpassword']=='Buy') {
	...
	$result1=mssql_query("SELECT ItemStock FROM CabalCash.dbo.WebShop Where ItemNum = '$ItemNum'");
...
if($error==1) {
echo "<table width=600 CELLSPACING=2 BORDER=0 CELLPADDING=2 align=center>";
$perRow=2;
$r=0;
$resTop1 = mssql_query("SELECT ProductNum, ItemMain, ItemSub, Duration, ItemStock, ItemMoney, ItemImage,ItemName,ItemComment FROM RanShop.dbo.ShopItemMap where Category = '37'");
...
<td><td><br><div align='center'><img src='items/$result1[ItemImage]' alt='$result1[ItemImage]' width='90' height='90'></div></td>
            <td width='130' class=alt1 align='left'><font color='white' size=1>Name: <b>$result1[ItemName]</b>
<br>Duration: $Duration1<br>Description: $result1[ItemComment]<br><br>Stock: <b>$result1[ItemStock]</b><br>Price: <b>$result1[ItemMoney]</b></font><br><form name='buy' action='webshop.php?op=show2' method='post' onsubmit='return checkform()' autocomplete='off'>
