...
function getC ($int) {
    ...
    if ($int == 16) { $char = "Scientist(M)"; }
    if ($int == 32) { $char = "Scientist(F)"; }
return $char;
}
function getG ($int) {
    if ($int == "0") { $char = "NONE"; }
...
$i=0;
$resTop = mssql_query("SELECT TOP 50 P.ChaName, P.ChaLevel, P.ChaPK2, P.ChaSchool, P.ChaOnline, P.ChaIntel, P.GuNum, P.ChaNum FROM RanGame1.dbo.ChaInfo P, RanUser.dbo.UserInfo U WHERE P.UserNum = U.UserNum AND U.UserType != 30 AND P.ChaDeleted != 1 ORDER BY P.ChaPK2 DESC, P.ChaLevel DESC");
while($result = mssql_fetch_array( $resTop )) {
	$i++;		

			$Name = $result["ChaName"];
			$lev = $result["ChaLevel"];
			$Reborn = $result["ChaIntel"];
			$ol = $result["ChaOnline"];
			$Num = $result["ChaNum"];
			$id = $result["GuNum"];
    			if($class == 1) {
...
echo"  						<tr class='li01' height='25'>
							...
							<td width='100' align='center' style='border-bottom:#CCCCCC 1px dashed'>$Reborn</td>
